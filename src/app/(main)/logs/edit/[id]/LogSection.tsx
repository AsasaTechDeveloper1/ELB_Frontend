...
export default function LogSection({
  ...
  const logId = params?.id as string; // ✅ "oQPrIEYJXQR3GiKJGe5V"
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      for (const entry of logEntries) {
        // 🔹 Override ID from URL if it's 0
        const effectiveId = logId;
        console.log(effectiveId,"effectiveId")
        const payload = { ...entry, id: effectiveId };

        const res = await fetch(`${API_BASE}/logs/${effectiveId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });

        if (!res.ok) throw new Error(`Failed to update log ${effectiveId}`);
        const data = await res.json();
        console.log(`✅ Updated log ${effectiveId}:`, data);
      }
                          ...
                          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full md:w-[320px]">
                            ...
                            <div className="flex flex-col w-full sm:w-[350px] max-w-[350px]">
                              <select
                                ...
                                value={entry.class}    // ✅ keep selected from API
                                onChange={(e) => {
                              ...
                              >
                                {["Class A", "Class B", "Class C"].map((opt) => (
