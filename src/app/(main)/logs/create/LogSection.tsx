  ...
  const handleLogInputChange = async (index: number, field: keyof LogEntry, value: any) => {
    ...
    if (field === 'ddNo' && ['Worked (W)', 'Cleared (C)'].includes(updatedEntries[index].ddAction)) {
      ...
      if (selectedDeferral) {
        ...
        updatedEntries[index].ddType = displayDdType(deferralData.dd_type || '');
        updatedEntries[index].melCdlRef = deferralData.mel_cdl_ref || '';
        updatedEntries[index].cat = displayMelCat(deferralData.cat || '');
      }
    }
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 shadow w-full max-w-[1200px] flex flex-col">
        <div className="flex justify-end mb-4">
          <button
            ...
            className="bg-[#004051] text-white px-6 py-2 rounded-md hover:bg-[#00363f] disabled:opacity-50"
          >
