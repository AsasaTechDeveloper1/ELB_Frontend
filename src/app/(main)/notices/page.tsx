...
export default function NoticesForm() {
    ...
    const fetchNotices = async () => {
      ...
      try {
        ...
        setAuthorizedEntries(
          fetchedEntries.map((entry, idx) => (entry.enteredAuth ? idx : -1)).filter(i => i !== -1)
        );
        setClearedEntries(
          fetchedEntries.map((entry, idx) => (entry.clearedAuth ? idx : -1)).filter(i => i !== -1)
        );
      } catch (err: any) {
  ...
  const removeEntry = (index: number) => {
    ...
    if (removed.id !== undefined) setRemovedIds(prev => [...prev, removed.id!]);
    setEntries(updated);
  ...
  /* ---------- SAVE NOTICES (with loader) ---------- */
  const handleSave = async () => {
    const hasEmptyDescription = entries.some(entry => !entry.description.trim());
    if (hasEmptyDescription) {
      const updatedErrors = entries.map(entry => (entry.description.trim() ? '' : 'Description is required'));
      setDescriptionErrors(updatedErrors);
    ...
    try {
      const res = await fetch(`${API_BASE}/notices`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ entries: payload, removedIds }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to save notices');

      alert('Notices saved successfully');
      window.location.reload(); // simple refresh – you could also refetch instead
    } catch (err: any) {
